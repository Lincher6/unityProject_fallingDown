...
{
    public GameObject stonePlatfrom;
    public GameObject spikePlatfrom;
    public GameObject breakablePlatfrom;
    public GameObject[] rotationalPlatform;
    ...
    void Start()
    {
        ...
        currentPlatform = Instantiate(stonePlatfrom, new Vector2(0, -4), Quaternion.identity).GetComponent<PlatformInfo>();
    ...
    private IEnumerator WindRoutine()
    {
        ...
        {
            Instantiate(wind[Random.Range(0, wind.Length)], transform.position, Quaternion.identity);
    ...
    private void SpawnPlatform()
    {
        if (Random.Range(0, 2) == 0)
        {
            currentPlatform = Instantiate(stonePlatfrom, spawnPosition, Quaternion.identity).GetComponent<PlatformInfo>();
        }
        else if (Random.Range(0, 5) == 0 && !currentPlatform.isSpike && !currentPlatform.isBreakable)
        {
            currentPlatform = Instantiate(spikePlatfrom, spawnPosition, Quaternion.identity).GetComponent<PlatformInfo>();
        }
        else if (Random.Range(0, 3) == 0)
        {
            currentPlatform = Instantiate(rotationalPlatform[Random.Range(0, 2)], spawnPosition, Quaternion.identity).GetComponent<PlatformInfo>();
        }
        else if (Random.Range(0, 2) == 0 && !currentPlatform.isBreakable && !currentPlatform.isSpike && isFirstPart)
        {
    ...
    private void SpawnStar()
    {
        ...
        {
            Instantiate(star, new Vector2(currentPosition.x, currentPosition.y + offset), Quaternion.identity);
        }

        if (Random.Range(0, 2) == 0 && !currentPlatform.isSpike)
        {
            Instantiate(star, new Vector2(Random.Range(minX, manX), currentPosition.y + offset + 0.5f), Quaternion.identity);
        }
    }
