...
public class SpawnManager6 : MonoBehaviour
{
    ...
    private IEnumerator BalloonRoutine()
    {
        while (!gm.IsGameStopped())
        {
            yield return new WaitForSeconds(20);
            Instantiate(waterBalloon, new Vector2(Random.Range(minX, manX), transform.position.y), Quaternion.identity);
    ...
    private IEnumerator TopFallingRoutine()
    {
        ...
        while (!gm.IsGameStopped())
        {
            Lava.instance.ShootLavaBall(Random.Range(minX, manX));
    ...
    private IEnumerator CanonRoutine()
    {
        ...
        while (!gm.IsGameStopped())
        {
            Instantiate(canon, new Vector2(0, transform.position.y), Quaternion.identity);
            yield return new WaitForSeconds(20);
        }
    }
    ...
    private void SpawnPlatform()
    {
        ...
        {
            currentPlatform = Instantiate(icePlatform[Random.Range(0, icePlatform.Length)], spawnPosition, Quaternion.identity).GetComponent<PlatformInfo>();
        }
